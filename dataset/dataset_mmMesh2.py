...
from .dataset_config_clean import *
...
class RF3DPoseDataset(Dataset):
    def __init__(
        ...
        normalize_flag=True, main_modality="radar_points", scale_id = "p3", split_id = "s2",
    ):
        ...
        self.env_paths = {
            ...
            'joints':    os.path.join(self.cache_path, 'joints.lmdb'),
        }
        if self.use_image:
    ...
    def __getitem__(self, idx):
            ...
            radar_PC_selected, indicator_selected = self.find_temporal_radar_pc_lmdb(idx, T=4)
            radar_image_selected, _ = self.find_temporal_radar_image_lmdb(idx, T=4)
            ...
            radar_PC_seq = np.stack(radar_PC_selected, axis=0)  # shape: (T, 1000, 4)
