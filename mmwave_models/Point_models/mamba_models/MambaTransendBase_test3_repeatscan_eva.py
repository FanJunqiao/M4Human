...
class Mamba(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        print(self.args.d_model,self.trans_latent, self.mamb_latent)
        ...
        self.root_emb2 = nn.Linear(self.args.vocab_size, self.args.temp_emb_dim)
    ...
    def forward(self, x, timesteps, mod=None, root = None, sqrt_alpha_hat = None):
        ...
        limb_emb = self.limb_embed2(limb.reshape(B, -1)).unsqueeze(dim=1)
        emb = (emb + limb_emb)
        # print("emb", emb.shape)
        ...
        if mod is not None:
            ...
            root_emb = self.root_emb2(root[:,0,:]).unsqueeze(dim=1)
            emb = (emb + mod_proj + root_emb)
