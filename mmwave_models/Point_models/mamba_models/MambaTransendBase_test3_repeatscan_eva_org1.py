...
class Mamba(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.end_embedding = nn.Linear(3, self.joint_latent_dim)
        self.cond_embed2 = nn.Linear(self.args.vocab_size * self.args.cond_length, self.args.temp_emb_dim)
        self.limb_embed2 = nn.Linear(14, self.args.temp_emb_dim)
    ...
    def forward(self, x, timesteps, mod=None, root=None, sqrt_alpha_hat = None):
        ...
        if mod is not None:
            # # === CADS only during inference ===
            # if not self.training:
            #     # Normalize timestep to [0,1]
            #     t_norm = timesteps.float() / self.args.cfg.noise_steps  # assumes timesteps are 0...T-1
            #     t_norm = t_norm.mean().item()  # assume scalar t, or modify if needed
            #     mod = apply_condition_annealing(mod.reshape(B, -1), t_norm)
            mod_proj = self.cond_embed2(mod.reshape(B, -1))#
            emb = (emb + mod_proj).unsqueeze(dim=1)
        # else:
        #     limb = compute_limb_lengths(root[:,0,:])
        #     limb_emb = self.limb_embed2(limb.reshape(B, -1))
        #     emb = (emb + limb_emb).unsqueeze(dim=1)
        #     # mod_proj = self.root_embed2(root.reshape(B, -1))#
        #     # emb = (emb + mod_proj).unsqueeze(dim=1)

        h = torch.cat([self.embedding(x[:,:-1,:]), self.end_embedding(x[:,[-1],:])], dim=1)
...
def apply_condition_annealing(mod, t, s=0.9, tau1=0.1, tau2=1.0, rescale=True, psi=1.0):
    """
    Apply condition annealing by injecting time-scheduled Gaussian noise into the condition.

    Args:
        mod (Tensor): Conditioning tensor of shape (B, ...), e.g., embedding or latent.
        t (float): Normalized current timestep in [0, 1].
        s (float): Noise scale factor (default: 0.25).
        tau1 (float): Start of annealing transition (full noise if t < tau1).
        tau2 (float): End of annealing transition (no noise if t > tau2).
        rescale (bool): Whether to normalize the perturbed mod to preserve original distribution.
        psi (float): Mixing factor for rescaling (1.0 = fully rescaled, 0.0 = raw corrupted).

    Returns:
        Tensor: Noised (and optionally rescaled) condition tensor.
    """

    # Compute annealing weight γ(t)
    if t <= tau1:
        gamma = 1.0
    elif t >= tau2:
        gamma = 0.0
    ...
    # Compute weighted condition + noise
    noise = torch.randn_like(mod)
    scale_cond = gamma ** 0.5
    scale_noise = (1.0 - gamma) ** 0.5
    mod_noised = scale_cond * mod + s * scale_noise * noise

    print(t, scale_cond, scale_noise)

    # Optional rescaling to match original statistics
    if rescale:
        mu_in = mod.mean(dim=1, keepdim=True)
        std_in = mod.std(dim=1, keepdim=True) + 1e-6  # Prevent division by zero
        mu_out = mod_noised.mean(dim=1, keepdim=True)
        std_out = mod_noised.std(dim=1, keepdim=True) + 1e-6

        mod_rescaled = (mod_noised - mu_out) / std_out * std_in + mu_in
        mod_noised = psi * mod_rescaled + (1.0 - psi) * mod_noised
