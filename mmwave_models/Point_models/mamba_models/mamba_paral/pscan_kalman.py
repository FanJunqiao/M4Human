...
#     @staticmethod
#     def pscan(A, X, kalman_func=None, kalman_apply = True):
#         # A : (B, D, L, N)
#         # X : (B, D, L, N)
...
#             Aa[:, :, :, 1].mul_(Aa[:, :, :, 0])
#             if kalman_apply == True and kalman_func != None:
#                 # a test
#                 Xa = Xa.permute(0,2,1,3,4).view(B* (T//2), D, 2, -1)
#                 Xa_copy = Xa_copy.permute(0,2,1,3).view(B* (T//2), D, -1)
#                 # Xa[:, :, 1].add_(Xa_copy)
#                 a, b = kalman_func(Xa[:, :, 1], Xa_copy)
#                 Xa[:, :, 1] = a+b
#                 Xa = Xa.view(B, (T//2), D, 2, -1).permute(0,2,1,3,4)
#             else:
#                 Xa[:, :, :, 1].add_(Xa_copy)
...
#         if kalman_apply == True and kalman_func != None:
#             # a test
#             X = X.permute(0,2,1,3).view(B*T, D, -1)
#             X_copy = X_copy.permute(0,2,1,3).view(B*T, D, -1)
#             # X.add_(X_copy)
#             a, b = kalman_func(X, X_copy)
#             X = a+b
#             X = X.view(B, T, D, -1).permute(0,2,1,3)
#         else:
...
def pscan(A_in, X_in, kalman_func=None, kalman_apply = True):
    """
    Applies the parallel scan operation, as defined above. Returns a new tensor.
    If you can, privilege sequence lengths that are powers of two.
    ...
    # parallel scan (modifies X in-place)
    A, X = pscan_func(A, X, kalman_func=kalman_func, kalman_apply=kalman_apply)
    
    # slice [:, :L] (cut if there was padding)
    return X.transpose(2, 1)[:, :L]


def pscan_func(A, X, kalman_func=None, kalman_apply = True):
    # A : (B, D, L, N)
    # X : (B, D, L, N)
    ...
    for k in range(num_steps):
        ...
        if kalman_apply == True and kalman_func != None:
            ...
            Xa_copy = Xa_copy.permute(0,2,1,3).view(B* (T//2), D, -1)
            # Xa[:, :, 1].add_(Xa_copy)
            # print("be",Xa[:, :, 1], Xa_copy)
        
            a, b = kalman_func(Xa[:, :, 1], Xa_copy)
            
            # print("af",a-Xa[:, :, 1], b-Xa_copy)
...
# output = torch.mean((y-pscan(a,x,kalman_func=kal_fuc)))
# print(x,a, pscan(a,x,kalman_func=kal_fuc))
# x.retain_grad()
# a.retain_grad() 
# y.retain_grad()
# output.backward()
