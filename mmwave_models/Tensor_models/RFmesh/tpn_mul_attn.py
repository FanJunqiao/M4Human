...
class MultiHeadAttentionModule_stride(nn.Module):
    """
    Input:  x ∈ [B, C, D, H, W]
    - Q/K/V: Conv3d(kernel=3,stride=(1,3,3),padding=1) → downsample H,W by /3
    - Compute multi-head attention on reduced tokens (D * H' * W')
    - Deconv back to (D,H,W) with ConvTranspose3d
    - Residual connection
    """
    def __init__(self, in_channels, num_heads=8, kernel_size=3):
        super().__init__()
        ...
        # Downsample
        self.query_conv = nn.Conv3d(in_channels, in_channels, kernel_size=k, stride=stride, padding=pad, bias=False)
        self.key_conv   = nn.Conv3d(in_channels, in_channels, kernel_size=k, stride=stride, padding=pad, bias=False)
        self.value_conv = nn.Conv3d(in_channels, in_channels, kernel_size=k, stride=stride, padding=pad, bias=False)

        # Upsample
        self.deconv = nn.ConvTranspose3d(in_channels, in_channels, kernel_size=k, stride=stride, padding=pad, bias=False)
    ...
    def forward(self, x):
        ...
        B, C, Dp, Hp, Wp = q.shape
        T = Dp * Hp * Wp

        # --- Reshape into heads ---
        q = q.view(B, self.num_heads, self.head_dim, T).permute(0,1,3,2)     # [B,heads,T,head_dim]
        k = k.view(B, self.num_heads, self.head_dim, T)                      # [B,heads,head_dim,T]
        v = v.view(B, self.num_heads, self.head_dim, T).permute(0,1,3,2)     # [B,heads,T,head_dim]

        # --- Attention ---
        energy = torch.matmul(q, k) / (self.head_dim ** 0.5)                 # [B,heads,T,T]
        attn = F.softmax(energy, dim=-1)
        out = torch.matmul(attn, v)                                          # [B,heads,T,head_dim]

        # --- Merge heads ---
        out = out.permute(0,1,3,2).contiguous().view(B, C, Dp, Hp, Wp)

        # --- Upsample to (D,H,W) ---
        out = self.deconv(out, output_size=(D, H, W))
...
class FPN3DWithMultiHeadAttention(nn.Module):
    def __init__(self, in_channels_list, out_channels, num_heads=8, stride_perform = False):
        ...
        for in_channels in in_channels_list:
            ...
            if stride_perform:
                attention_module = MultiHeadAttentionModule_stride(out_channels, num_heads)
            else:
...
class Simple3DConvModelWithTripleCNNFPNAndAttention(nn.Module):
    def __init__(self, smplx_model_paths, input_channels=31, fpn_out_channels=256, reduced_channels=256, dropout_rate=0.0):
        super(Simple3DConvModelWithTripleCNNFPNAndAttention, self).__init__()

        '''Detection part of model'''
        self.conv1 = nn.Conv3d(31, 64, kernel_size=3, stride=2, padding=1)
        ...
        self.fpn = FPN3DWithMultiHeadAttention([64, 128, 256], fpn_out_channels, stride_perform=True)

        # 重新计算展平后的尺寸：128 * 1 * 16 * 14
        self.conv1x1 = nn.Conv3d(fpn_out_channels, reduced_channels, kernel_size=1)
        self.maxpool_trans = nn.MaxPool2d(kernel_size=3, stride=3)
        ...
        self.register_buffer("tensor_offset_min", torch.tensor([16, 16, 12]))
        self.register_buffer("tensor_offset_max", torch.tensor([16, 16, 12]))
        ...
        self.fpn_m = FPN3DWithMultiHeadAttention([64, 128, 256], fpn_out_channels*2, stride_perform=False)
    ...
    def forward(self, in_tensor):
        ...
        x = self.maxpool_trans(x.squeeze(2))
        # if torch.isnan(x).any():
        #     print("NaN detected in x after maxpool")
        # print(x.shape)
        ...
        pred_trans_norm = self.fc2_trans(x)
        pred_trans = pred_trans_norm * self.trans_scale + self.trans_offset
        ...
        x_crop, crop_start = self.crop_5d_asym_fixed(in_tensor, pred_trans_norm)
        ...
        fpn_outs_m = self.fpn_m([c_crop1, c_crop2, c_crop3])
        # print(fpn_outs_m[-1].shape)
        
        x_m = F.relu(self.conv1x1_m(fpn_outs_m[-1]))
        # print(x_m.shape)
        # x_m = self.maxpool_trans_m(x_m)
        # print(x_m.shape)
        # print(x_m.shape)
