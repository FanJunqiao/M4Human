...
class FPN3DWithMultiHeadAttention(nn.Module):
    ...
    def forward(self, inputs):
        ...
        laterals = [output_conv(x) for output_conv, x in zip(self.output_convs, laterals)]
        return laterals
...
class Simple3DConvModelWithTripleCNNFPNAndAttention(nn.Module):
    def __init__(self, smplx_model_paths, input_channels=(4,121,111,31), fpn_out_channels=256, reduced_channels=256, dropout_rate=0.0):
        ...
        ], fpn_out_channels, stride_perform=True)

        # 重新计算展平后的尺寸：128 * 1 * 16 * 14
        self.conv1x1 = nn.Conv3d(fpn_out_channels, reduced_channels, kernel_size=1)
        # self.maxpool_trans = nn.MaxPool3d(kernel_size=first_layer_size, stride=first_layer_size)
    ...
    def forward(self, in_tensor):
        ...
        x = F.relu(self.conv1x1(fpn_outs[0]))
        ...
        # print(x_m.shape)
        # print(x_m.shape)
        x_m = x_m.view(x_m.size(0), x_m.size(1), -1)
        x_m = F.max_pool1d(x_m, kernel_size=x_m.size(-1)).squeeze(-1)
        # print(x_m.shape)
        x_m = self.fc1_m(x_m)
